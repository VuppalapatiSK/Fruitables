...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        updateQuantity(state,action){
            ...
            const itemUpdate = state.data.find((item)=> item.id === id);
            if(itemUpdate){
                const validQuanity = Math.max(quantity || 1,1);
                itemUpdate.quantity = validQuanity;
                itemUpdate.totalPrice = itemUpdate.price * quantity;
            }
        },
        removeItem(state, action){
            const tempCart = state.data.filter((item)=>(
                item.id !== action.payload.id
            ));
            state.data = tempCart;
            storeInLocalStorage(state.data);
